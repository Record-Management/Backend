...
public class HabitRecordController {
    ...
    @Operation(summary = "습관기록 작성", 
               ...
               ## 메인 습관 기록 자동 생성 시스템 (v2.0.0 스케줄러 방식)

               **매일 자정 스케줄러 자동 생성**:
               - 매일 자정(00:00) 한국시간 기준으로 실행
               - 습관 타입 사용자(mainRecordType=HABIT)의 오늘 날짜 메인 습관 기록 자동 생성
               - 습관 기간 내에 있고, 메인 습관 기록이 없는 경우에만 생성
               - 사용자가 앱에 진입하지 않아도 자동으로 생성됨
               - 배치 처리(100명 단위)와 벌크 INSERT로 성능 최적화
               - 어제 메인 기록과 동일한 habitType으로 생성 (첫 습관은 WATER_DRINKING)

               **자동 생성 동작 방식**:
               ```
               예시: 11월 19일에 30일 목표 메인 습관 기록 작성
               → 11월 19일: 사용자가 직접 작성
               → 11월 20일: 자정 스케줄러가 자동 생성
               → 11월 21일: 자정 스케줄러가 자동 생성
               → ... (매일 자정마다 자동 생성)
               ```

               **2단계 시스템**:
               - **미완료 (자동생성)**: isCompleted=false → 회색 아이콘
               ...
               **알림 발송 시스템**:
               ...
               - **알림 히스토리**: 발송 성공 시 `HABIT_TIME_BASED_REMINDER` 타입으로 히스토리에 저장됨
               
               ## 메인/서브 기록 결정 로직 (우선순위)
               1. **사용자 명시적 설정 최우선**: isMainRecord=true 설정 시 메인 기록으로 생성 (서브→메인 변경 시)
               2. **명시적 서브 설정**: isMainRecord=false 설정 시 서브 기록으로 생성
               3. **습관 타입 사용자 첫 습관**: 해당 날짜에 메인 습관이 없고 습관 타입 사용자면 메인 기록 자동 설정
               4. **기본값**: 이미 메인 습관이 있거나 다른 타입 사용자면 서브 기록
               
               ## 기록 제한 사항
               - 하루 전체 기록(일상+운동+습관) 합쳐서 최대 2개, 최대 2가지 기록 타입
               - 예시: 습관 1개 + 일상 1개 = 2개 (가능) / 습관 2개 + 운동 1개 = 3개 (불가능)
            ...
            content = @io.swagger.v3.oas.annotations.media.Content(
                ...
                examples = {
                    @io.swagger.v3.oas.annotations.media.ExampleObject(
                        ...
                        summary = "습관 타입 사용자의 첫 습관은 isMainRecord 값에 관계없이 무조건 메인 기록 + 내일부터 매일 자정 스케줄러가 자동 생성",
                        value = """
                            {
                              "statusCode": 200,
                              "code": "SUCCESS",
                              ...
                              "data": {
                                ...
                                "memo": "30일 물마시기 도전 시작! (첫 습관은 무조건 메인, 내일부터 스케줄러가 자동 생성)",
                                "isCompleted": false,
                                "isMainRecord": true
                              }
                            }
    ...
    @Operation(summary = "습관기록 수정", 
               ...
               **서브→메인 변경 시 자동 처리:**
               1. **오늘자 기존 메인**: 삭제되지 않고 서브 기록으로 변경 (완료상태/내용 모두 유지)
               2. **내일부터 기존 메인**: 목표 종료일까지 기존 메인 습관 기록 삭제
               3. **새로운 메인 생성**: 내일부터 매일 자정 스케줄러가 새로운 메인 습관 기록 자동 생성
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "200",
        description = "습관기록 수정 성공 (서브→메인 변경 시 기존 메인 삭제, 내일부터 스케줄러가 자동 생성)",
        content = @io.swagger.v3.oas.annotations.media.Content(
            mediaType = "application/json",
            examples = @io.swagger.v3.oas.annotations.media.ExampleObject(
                name = "메인 습관기록 수정 성공",
                summary = "메인 습관기록 수정 성공 - 서브→메인 변경 시 기존 메인 삭제, 내일부터 스케줄러가 자동 생성",
                value = """
                {
                    "statusCode": 200,
                    "code": "S20000",
                    ...
                    "data": {
                        ...
                        "memo": "서브 독서가 메인으로 승격! (기존 메인 삭제, 내일부터 스케줄러가 자동 생성)",
                        "isCompleted": false,
                        "isMainRecord": true
                    }
                }
    ...
    @Operation(summary = "습관기록 삭제", 
               ...
               **메인 습관 기록 삭제 (v1.8.5):**
               - **메인+서브 상황**: 오늘부터 목표 종료일까지 기존 메인 기록 삭제 → 서브 기록 중 하나가 메인으로 전환 → 내일부터 매일 자정 스케줄러가 새 메인 습관 자동 생성
               - **메인만 상황**: 메인 기록 삭제 시 **목표 기간 전체(시작일~종료일)의 해당 습관 모든 기록**이 삭제됩니다.

               **중요한 삭제 동작 (정리본 반영):**
               - 메인+서브 → 메인 삭제 시: 오늘부터 기존 메인 습관 완전 삭제, 서브가 메인으로 승격하여 내일부터 스케줄러가 자동 생성
               - 메인만 → 메인 삭제 시: **습관 포기로 간주**하여 목표기간 전체의 해당 습관 모든 기록이 삭제
               - **되돌릴 수 없는 작업**이므로 신중하게 사용해주세요.

               **자동 처리 상세:**
               - 메인+서브 상황: 메인 습관 기록 삭제 → 오늘부터 목표 종료일까지 기존 메인 기록 삭제 → 서브가 메인으로 전환 → 내일부터 매일 자정 스케줄러가 새 메인 습관 자동 생성
        ...
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            ...
            content = @io.swagger.v3.oas.annotations.media.Content(
                ...
                examples = {
                    ...
                    @io.swagger.v3.oas.annotations.media.ExampleObject(
                        name = "메인+서브 상황 메인 삭제",
                        summary = "메인 기록 삭제 시 오늘부터 기존 메인 삭제 → 서브→메인 전환 → 내일부터 스케줄러가 자동 생성",
                        value = """
                            {
                                "statusCode": 200,
                                "code": "S20000",
